...
```bash
curl -X POST http://localhost:3000/api/login -H "Content-Type: application/json" -d '{"email":"admin@library.com"}' -c cookies.txt
```
...
Create a loan (transactional):

```bash
curl -X POST http://localhost:3000/api/loans -H "Content-Type: application/json" -d '{"UserID":1,"DueDate":"2025-12-15","ISBNs":["978-0-123456-47-2"]}' -b cookies.txt
```

Use debug header instead of login (no cookies):

```bash
curl -H "X-Debug-Auth: true" -H "X-Debug-User-Email: dev@local" http://localhost:3000/api/admin/summary
```
